            ...
            for(shape in shapes) {
                val reshaped = shape.map { contour ->
                    val points = (0 until contour.segments.size).map {
                        ...
                        val d = (position - cursor).length

                        val dv = (position-cursor)
                        val da = Math.atan2(dv.x, dv.y)*8.0

                        position + normal * Math.cos(seconds * 10.0 + da) * 10.0
                    }
...
fun main(args: Array<String>) {
    application(Letters002(), configuration {
        width = 1280
        height = 720
    })
}